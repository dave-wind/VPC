...
function dave() {
  ...
  function l() {
    ...
    return {l: w, z: o(v, "zIndex", -1), c: o(v, "color", "255,255,255"), n: o(v, "count", 99)}
  }
    ...
    t.forEach(function (i) {
      ...
      for (v = 0; v < w.length; v++) {
        ...
        if (i !== x && null !== x.x && null !== x.y) {
          ...
          y < x.max && (x === f && y >= x.max / 2 && (i.x -= 0.03 * B, i.y -= 0.03 * z), A = (x.max - y) / x.max, e.beginPath(), e.lineWidth = A, e.strokeStyle = "#fff", e.moveTo(i.x, i.y), e.lineTo(x.x, x.y), e.stroke())
        }
      }
  ...
  u.id = c;
  u.style.cssText = "position:fixed;top:0;left:0;z-index:" + s.z + ";background-color: #56bc8a";
